  ...
  module Cards
        ...
        decks.each do |deck|
          ...
          wasabi_indexes.each do |i|
            ...
            nigiris_after_this_wasabi.each do |j|
              next if scored_nigiris.include? j
              scored_nigiris << j
    ...
    class Pudding < Dessert
      ...
      def self.score_dessert(decks)
        ...
        # scored the same
        return [0] * decks.size if max == min
        ...
        pudding_counts.each do |count|
          ...
          scores << if count == max
                      (6.0 / max_puddings_player_count).floor
                    # if you were the least scoring player and
                    # there were more than 2 players
                    elsif count == min && decks.count > 2
                      (-6.0 / min_puddings_player_count).ceil
                    else
                      0
                    end
        end
    ...
    class Maki < Card
      ...
      def self.score_round(decks)
        ...
        all_counters.each do |count|
          scores << if count > 0 && count == highest_counter
                      (6 / number_highest).floor
                    elsif count > 0 && count == second_highest_counter
                      (3 / number_second_highest).floor
                    else
                      0
                    end
        end
